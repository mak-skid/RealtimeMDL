...
\[H_t = O_t \odot \tanh(C_t)\]
where $*$ denotes the convolution operator. Note that $X_t$, $C_t$, $H_t$, $I_t$, $F_t$, $O_t$ $\in$ $R_{H \times W \times L}$, where $R_{H \times W \times L}$ denotes the space of the 3D tensor, and $H$, $W$, and $L$ are the height, width, and each grid feature vector of the tensor, respectively. 
