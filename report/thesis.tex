...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=15cm]{{output1.png}} % CHANGE THIS FIGURE
    \caption{Visualisation of vehicle speed and density in each road section}
\end{figure}
...
Spark Structured Streaming\cite{10.1145/3183713.3190664}, an extension of Apache Spark built on top of its SQL engine, allow a micro-batch processing for incoming streams. This streaming process makes it suitable for real-time MDL model inference for which a batch of 4D tensor input is taken. In Spark Structured Streaming, every data item that is arriving on the stream can be considered as appending a new row to a \textit{input table}. A query on the input will generate the \textit{result table}, and with arrival of new rows, the result table gets updated incrementally. It allows time-window-based aggregation queries to be performed on the input table by watermarking, and handling late arrivals of input data is also supported, which makes it more resillient to a real-world setting. The result table can be written to various sinks such as Kafka, HDFS, and databases. Comparing to similar streaming data processing systems, Spark Structured Streaming throughput outperforms that of Apache Flink by up to two 2$\times$, Apache Kafka Streams by 90$\times$. Together with Kafka's persistent data storage, low-latency, and distributed scalable streaming, the design decision of combining Kafka with Spark Structred Streaming's high computational throughput and flexible integration of a machine learning model can be justified.

\begin{figure}[ht!]
    \centering
    \includegraphics[width=14cm]{{throughput_comp.png}}
    \caption{Throughput result comparison and scalability of Spark Structured Streaming on the Yahoo! benchmark. From Armbrust et al.}
\end{figure}
...
\[ MAPE = \frac{1}{n}\sum_{i=1}^{n}\frac{|\hat{Y_i} - Y_i|}{Y_i} \] % MAYBE DROP MAPE SINCE IT SHOWS VERY HIGH VALUE, The MAPE is not suitable for the evaluation of the model since the denominator is the actual speed, which can be zero.
...
\begin{table}[ht!]
    ...
    \begin{tabular}{ |P{4cm}|P{3cm}|P{2cm}|P{2cm}|P{2cm}| }
        ...
        \hline
        \multirow{4}{*}{Original MDL} & \multirow{4}{*}{speed, density} & \multirow{4}{*}{2} & 0.5 sec & Yes\\
         & & & 0.5 sec & No\\
         & & & 10 sec & Yes\\
         & & & 10 sec & No\\
        \hline
        \multirow{4}{*}{Downsized MDL} & \multirow{4}{*}{speed} & \multirow{4}{*}{1} & 0.5 sec & Yes\\
         & & & 0.5 sec & No\\
         & & & 10 sec & Yes\\
         & & & 10 sec & No\\ 
        \hline
...
\section{Accuracy}
The results for accuracy are shown in Table 4.1-4.2. % DOUBLE CHECK THE TABLE NUMBER ONE LAST TIME
...
% real time model performs worse than the static model, which is due to the loss of datapoints during streaming
